...
const friendlySemanticSummary = 'Automatic analysis complete: {detectionDebug.candidateMasks} architectural mask{detectionDebug.candidateMasks === 1 ? "" : "s"} · semantic object proposals + boundary refinement · {zones.filter((zone) => (zone.label ?? "").startsWith("Auto architectural mask")).map((zone) => (zone.label ?? "").replace("Auto architectural mask · ", "")).join(", ") || "no confident architectural objects"}{detectionDebug.semanticWarnings ? ` · ${detectionDebug.semanticWarnings} warning${detectionDebug.semanticWarnings === 1 ? "" : "s"}` : ""}';
const priorFriendlySemanticSummary = 'Automatic detection: {detectionDebug.candidateMasks} architectural mask{detectionDebug.candidateMasks === 1 ? "" : "s"} · semantic object proposals + boundary refinement · {zones.filter((zone) => (zone.label ?? "").startsWith("Auto architectural mask")).map((zone) => (zone.label ?? "").replace("Auto architectural mask · ", "")).join(", ") || "no confident architectural objects"}{detectionDebug.semanticWarnings ? ` · ${detectionDebug.semanticWarnings} warning${detectionDebug.semanticWarnings === 1 ? "" : "s"}` : ""}';
...
} else if (source.includes(priorFriendlySemanticSummary)) {
  source = source.replace(priorFriendlySemanticSummary, friendlySemanticSummary);
  await fs.writeFile(path, source);
  console.log("Clarified that automatic semantic analysis has already completed.");
} else if (source.includes(semanticSummary)) {
  source = source.replace(semanticSummary, friendlySemanticSummary);
  await fs.writeFile(path, source);
  console.log("Replaced semantic detector status with a user-friendly feature summary.");
