...
assert.equal(supportSpacingIsSafe([42, 30, 29, 15.4]), true, "one moderately expanded noisy interval alone should remain eligible");
assert.equal(supportSpacingIsSafe([68, 30, 28, 15.4]), false, "a boundary missing-opening gap must survive one separate expanded noisy interval");
assert.equal(supportSpacingIsSafe([68, 30, 49, 15.4]), false, "two missing-opening-sized gaps must not be excused as one expanded noisy interval");
assert.equal(supportSpacingIsSafe([42, 30, 43, 8.5, 11]), false, "an interior missing-opening gap must still survive separate noisy spacing");
assert.equal(supportSpacingIsSafe([25, 23, 20, 18]), true, "mild natural spacing variation should remain eligible");

console.log("robust perspective spacing expanded-noise smoke passed");
