...
assert.equal(supportSpacingIsSafe([42, 30, 29, 15.4]), true, "one distant expanded noisy interval should remain eligible");
assert.equal(supportSpacingIsSafe([42, 30, 13, 15.4]), true, "one distant compressed noisy interval should remain eligible");
...
assert.equal(supportSpacingIsSafe([12, 18, 28, 20]), true, "a suspected missing gap backed only by immediately adjacent expanded noise should stand down");
assert.equal(supportSpacingIsSafe([68, 30, 49, 15.4]), false, "two missing-opening-sized gaps must remain rejected by stronger evidence");

console.log("adjacent spacing-noise guard smoke passed");
