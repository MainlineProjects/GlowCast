...
const oldRowLike = `const rowLike = rowSpread <= Math.max(bounds.height * 0.08, Math.min(...members.map((member) => member.box.height)) * 0.55);`;
const oldColumnLike = `const columnLike = columnSpread <= Math.max(bounds.width * 0.08, Math.min(...members.map((member) => member.box.width)) * 0.55);`;

const newRowLike = `const minMemberWidth = Math.min(...members.map((member) => member.box.width));
...
const newColumnLike = `const columnLike = columnSpread <= Math.max(bounds.width * 0.08, minMemberWidth * 0.55) || perspectiveSlopedColumn;`;

if (source.includes(oldRowLike) && source.includes(oldColumnLike)) {
  source = source.replace(oldRowLike, newRowLike).replace(oldColumnLike, newColumnLike);
